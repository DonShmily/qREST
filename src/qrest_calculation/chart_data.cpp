...
**
** Project: qREST
** File: \src\qrest_calculation\chart_data.cpp
** -----
** File Created: Monday, 26th August 2024 09:35:18
** Author: Dong Feiyue (donfeiyue@outlook.com)
** -----
** Last Modified: Monday, 4th November 2024 01:18:05
...
void ChartData::CalculateGmp(std::size_t idx)
{
    ...
    gmp_[cur_dir_] = gmp_calculation::GmpCalculation(
        data_interface_->acc_[cur_dir_].col(cur_idx_), config_);
}
...
void ChartData::CalculateEdpFi()
{
    ...
    fi_[cur_dir_] = edp_calculation::FilteringIntegral(
        data_interface_->acc_[cur_dir_], data_interface_->building_, config_);
...
void ChartData::CalculateSafty()
{
    ...
    if (safty_idr_[cur_dir_].is_calculated()
        && safty_acc_[cur_dir_].is_calculated())
    {
        return;
    }
    CalculateEdpMfi();
    safty_idr_[cur_dir_] = safty_tagging::BasedOnInterStoryDrift(
        mfi_[cur_dir_].get_result_ptr(), config_);
    safty_idr_[cur_dir_].TagSafty();
    safty_acc_[cur_dir_] = safty_tagging::BasedOnAcceleration(
        mfi_[cur_dir_].get_result_ptr(), config_);
...
// 获取指定楼层层间位移角时程数据
ChartData::points_vector ChartData::get_idr(std::size_t idx)
{
    if (config_->edp_method == 0)
    {
        // 计算滤波积分
        CalculateEdpFi();
        edp_result_ = fi_[cur_dir_].get_result_ptr();
    }
    else
    {
        // 计算改进滤波积分
        CalculateEdpMfi();
        edp_result_ = mfi_[cur_dir_].get_result_ptr();
    }
    ...
    // 获取指定楼层层间位移角时程数据
    const auto &idr = edp_result_->get_inter_story_drift();
    return {time_, idr.get_col(idx)};
}

// 获取指定楼层位移时程数据
ChartData::points_vector ChartData::get_disp(std::size_t idx)
{
    if (config_->edp_method == 0)
    {
        // 计算滤波积分
    ...
    // 获取指定楼层位移时程数据
    const auto &disp = edp_result_->get_displacement();
    return {time_, disp.get_col(idx)};
}

// 获取指定楼层加速度时程数据
ChartData::points_vector ChartData::get_acc(std::size_t idx)
{
    if (config_->edp_method == 0)
    {
        // 计算滤波积分
    ...
    // 获取指定楼层加速度时程数据
    const auto &acc = edp_result_->get_acceleration();
    return {time_, acc.get_col(idx)};
}

// 获取层间位移角分布数据
ChartData::points_vector ChartData::get_all_idr()
{
    if (config_->edp_method == 0)
    {
        // 计算滤波积分
    ...
    // 获取层间位移角分布数据
    safty_idr_[cur_dir_] =
        safty_tagging::BasedOnInterStoryDrift(edp_result_, config_);
    safty_idr_[cur_dir_].TagSafty();
...
ChartData::points_vector ChartData::get_max_acc()
{
    ...
    // 获取最大楼面加速度数据
    safty_acc_[cur_dir_] =
        safty_tagging::BasedOnAcceleration(edp_result_, config_);
    safty_acc_[cur_dir_].TagSafty();
...
{
    time_.resize(data_interface_->data_config_.time_window_);
    for (std::size_t i = 0; i < time_.size(); ++i)
    {
        time_[i] = 1.0 * (i + data_interface_->time_idx_)
                   / data_interface_->data_config_.frequency_;
    }
}
