...
**
** Project: qREST
** File: \src\qrest_calculation\chart_data.h
** -----
** File Created: Monday, 26th August 2024 09:35:07
** Author: Dong Feiyue (donfeiyue@outlook.com)
** -----
** Last Modified: Monday, 4th November 2024 01:17:57
...
public:
    ...
    ChartData(std::shared_ptr<DataInterface> data_interface,
              std::size_t dir = 0)
        : data_interface_(data_interface), config_(data_interface->config_)
    {
        set_direction(dir);
        gmp_.resize(data_interface_->data_config_.direction_);
        spi_.resize(data_interface_->data_config_.direction_);
        fi_.resize(data_interface_->data_config_.direction_);
        mfi_.resize(data_interface_->data_config_.direction_);
        safty_idr_.resize(data_interface_->data_config_.direction_);
        safty_acc_.resize(data_interface_->data_config_.direction_);
    }
    ...
    // @param idx 楼层索引
    // @return 层间位移角时程数据序列
    points_vector get_idr(std::size_t idx);

    // 获取指定楼层位移时程数据
    // @param idx 楼层索引
    // @return 楼层位移时程数据序列指针
    points_vector get_disp(std::size_t idx);

    // 获取指定楼层加速度时程数据
    // @param idx 楼层索引
    // @return 楼层加速度时程数据序列指针
    points_vector get_acc(std::size_t idx);

    // 获取层间位移角分布数据
    // @return 层间位移角分布数据序列指针
    points_vector get_all_idr();
...
private:
    ...
    std::vector<edp_calculation::ModifiedFilteringIntegral>
        mfi_{}; // 改进滤波积分计算对象
    std::shared_ptr<edp_calculation::EdpResult> edp_result_{}; // EDP计算结果
    ...
    // 计算参数数据成员指针
    std::shared_ptr<settings::Config> config_{}; // 计算参数对象
